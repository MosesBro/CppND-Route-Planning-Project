...
// To sort, a function to compare F-values is needed.
bool compare_F(RouteModel::Node *a, RouteModel::Node *b) {
...
RouteModel::Node *RoutePlanner::NextNode() {
    // do some sorting
    std::sort(this->open_list.begin(), this->open_list.end(), compare_F);

    // pointer to the node in the list with the lowest sum.
    auto lowest_f = this->open_list.back();
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    while (current_node != this->start_node) {
        ...
        // get distance to parent node, and tally
        distance += current_node->distance(*parent_node);

        // add current node to path_found
        path_found.push_back(*current_node);

        // make the parent_node the current_node
...
void RoutePlanner::AStarSearch() {
    ...
    // need to prime the open_list with the start_node
    this->open_list.push_back(start_node);
    this->start_node->visited = true;

    while (!this->open_list.empty()) {
        current_node = this->NextNode();
        // current_node->distance(*this->end_node) == 0
